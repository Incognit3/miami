...
//    $(":button").dblclick(function(){alert("Вы произвели двойной щелчок по кнопке.")});
// });

// $(document).ready(function(){
  
//    $("#text1").change(function(){alert("Содержимое тестового поля было изменено.")});
//    $("#select1").change(function(){alert("Выбранный элемент был изменен.")});
...
// //       $("div").html("Размер окна браузер был изменен.");
// //    });
...
//       alert("Вы выделили текст в текстовом поле.");
//    });
...
//    alert("Документ был полностью загружен.");
...
// }); 
 ...
 //    if (
 //       $("#modal-window").hasClass('visible') //Метод .hasClass() возвращает true если у элемента есть искомый класс и false в обратном случае
 //       &&  e.target.className === 'modal-close' //идентифицирует элемент
 //      || !$("#modal-window")[0].contains(e.target) //Искомый текст может быть как непосредственно в каком-то элементе, так и в его потомках
 //    ) {
